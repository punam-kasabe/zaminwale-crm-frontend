    ...
    try {
      ...
      Object.keys(formData).forEach((key) => {
        if (
          key !== "panPhoto" &&
          key !== "aadharPhoto" &&
          key !== "bookingFormPhoto"
        ) {
      if (typeof formData[key] === "object") {
       data.append(key, JSON.stringify(formData[key])); // arrays, objects
      } else {
       data.append(key, formData[key]); // strings, numbers
       }
        }
      });
      ...
      if (formData.panPhoto)
        data.append("panPhoto", formData.panPhoto);

      if (formData.aadharPhoto)
        data.append("aadharPhoto", formData.aadharPhoto);

      if (formData.bookingFormPhoto)
        data.append("bookingFormPhoto", formData.bookingFormPhoto);

      data.append("user", loggedInUser);

      const baseURL = "http://192.168.29.50:5001";

      if (formData._id) {
        await axios.put(`${baseURL}/api/customers/${formData._id}`, data, {
          headers: { "Content-Type": "multipart/form-data" },
        });

        alert("✅ Customer updated successfully!");
      } else {
        await axios.post(`${baseURL}/api/customers`, data, {
          headers: { "Content-Type": "multipart/form-data" },
        });
...
>
        ...
        {currentStep === 1 && (
          <>
            ...
            <label>Booking Form Photo</label>
            <input
              type="file"
              accept="image/*"
              capture="environment"
              onChange={(e) =>
                setFormData({ ...formData, bookingFormPhoto: e.target.files[0] })
              }
            />
